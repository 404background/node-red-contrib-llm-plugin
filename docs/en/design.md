...
- A node checkpoint has no message to hang off, so `GET /llm-plugin/checkpoints` lists checkpoint headers (no flow bodies) newest-first, with `?source=node-apply` to narrow. A restore point nobody can find is not a restore point. The **Restore Points** dialog (the history button in the sidebar header) is the front end for it: a badge for which producer, what the checkpoint was taken before, when, how big the snapshot is, and which flows were in scope — resolved to flow *names*, since the ids mean nothing to someone deciding whether this is the point they want. The description has to identify **that** point, and is the name alone: the conversation's own title for a sidebar import, the node's name for a node edit. No sentence around it — the badge already says which producer it was, and a prefix only pushes the identifying part off the end of the line. Describing every sidebar import the same way turned the list into one repeated line, which is the same as having no description at all. A checkpoint whose chat has since been deleted reads "Sidebar import".
