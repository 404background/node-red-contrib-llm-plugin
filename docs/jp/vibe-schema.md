...
LLM が素の Node-RED JSON の代わりに生成する、エイリアスを鍵にした JSON。ランダムな ID、
ピクセル座標、入れ子になった接続配列 — LLM が意味のある形で発明できない3つの要素 — を
取り除いてある。素の Node-RED JSON との相互変換は変換器が受け持つ。
...
### 2. 等価な Node-RED JSON(変換器が生成するもの)
...
指定を実ノード ID へ解決している。[docs/jp/layout.md](./layout.md#コメント配置)を参照。
...
([docs/jp/architecture.md](./architecture.md) の `importFlowFromMessage`「プロパティ保存」を参照)。

したがって `disabled` と `showLabel` は予約名である。これらの名前のプロパティを
本当に持つノード型も壊れない。名前が埋まっているときは往路の持ち上げを行わず、
スキーマに `d` / `l` が直接書かれていればそちらを優先し、真偽値とみなせる値
...
ノード種別の**出力ポート数と各ポートの意味**。`connections` は任意の `fromPort`
を書けるが、その種別がポートをいくつ持つか、どれが何かはスキーマのどこにも現れない。
その情報はノードのエディタ定義側にあり、フローの JSON には含まれないからである。
馴染みのない複数出力ノードに対する現実的な答えは、**出力をすべて接続したサンプルを
フローコンテキストに置いておく**こと。モデルは見えている配線に倣う。コンテキストから
参照されていない config ノードについても同じことが言える([design.md §5](./design.md))。
...
関係ないからである。実際の配置の寸法は [docs/jp/layout.md](./layout.md#コメント配置)を参照。
...
6. **レイアウト** — まず論理的な位置(列・行)を求め、続いて幅を考慮して実座標に落とす。
   [docs/jp/layout.md](./layout.md)を参照。
