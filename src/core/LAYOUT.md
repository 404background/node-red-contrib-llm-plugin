...
| `LAYOUT_DEFAULTS` | `{ startX, startY, spacingX, spacingY, componentGap, maxColumns }` constants. |
...
LAYOUT_DEFAULTS = {
    startX:        60,
    startY:        60,
    spacingX:     180,   // horizontal distance between node centres
...
All three layout functions accept an `options` object that overrides any of
these per call. `placeAddedNodesNearNeighbors` also accepts `bandGap` (defaults
to `componentGap`) for the orphan band offset.
...
4. `computeComponentYOffsets` stacks components vertically with
   `componentGap` between them.
5. For each canvas node:
...
For each new node, examine the *already-positioned* neighbors:

| Situation | New node placement |
|-----------|--------------------|
| Both preds and succs positioned | `x = max(pred.x) + spacingX`, `y = mid( avg(pred.y), avg(succ.y) )` |
| Preds only positioned (appended) | `x = max(pred.x) + spacingX`, `y = avg(pred.y)` |
| Succs only positioned (prepended) | `x = min(succ.x) - spacingX`, `y = avg(succ.y)` |
...
Even with `x = max(pred.x) + spacingX`, the new node may now sit right on
top of an existing successor. For each `(new node N, existing succ S)`
pair, compute `needed = (N.x + spacingX) - S.x`. If `needed > 0`, BFS
forward through `outgoing` from S and shift every reachable node's `x` by
`needed`. The maximum shift wins when multiple paths converge.
...
#### Example — single insertion `A → N → B`

Before: `A(100, 100) → B(280, 100)` and the LLM proposes `A → N → B`.

| After | A | N | B |
|-------|---|---|---|
| Step 3 (no shift) | (100, 100) | (280, 100) | (280, 100) ← overlap |
| Step 3.4 | (100, 100) | (280, 100) | (460, 100) ← shifted +180 |

#### Example — chain insertion `A → N1 → N2 → B`

| After | A | N1 | N2 | B |
|-------|---|----|----|---|
| Step 3 | (100, 100) | (280, 100) | (460, 100) | (280, 100) |
| Step 3.4 | (100, 100) | (280, 100) | (460, 100) | (640, 100) ← shifted +360 |

### Step 3.5 — Vertical overlap resolution

Any remaining overlap (new node within `spacingX * 0.5` horizontally **and**
`spacingY * 0.8` vertically of any other positioned node) is resolved by
pushing the *new* node down by `spacingY`. Re-runs until stable, capped at
`newlyPlaced.length * 2` passes.
...
Layout.placeAddedNodesNearNeighbors(
    ...
    { spacingX: 180, spacingY: 80 }
);
// a stays at 100, n placed at 280, b shifted to 460.
